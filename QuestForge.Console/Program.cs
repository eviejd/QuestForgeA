...
void ProcessEvent()
{
    ...
    if (gameEvent.Type == EventType.Combat)
        Combat(gameEvent);
    else
        gameManager.ApplyEffects(gameEvent, zoneManager);
}
...
void EndGame()
{
    ...
    gameManager.SaveRankings(lbPath);
}

void Combat(GameEvent gameEvent)
{
    var difficulty = gameEvent.CombatDifficulty ?? Difficulty.Easy;

    var template = new Enemy(
        "Goblin",
        30,
        8,
        3,
    ...
    Console.WriteLine(
        $"A {enemy.Name} appears! HP:{enemy.Health} ATK:{enemy.Attack} DEF:{enemy.Defence}");
    ...
    while (player.IsAlive && enemy.IsAlive)
    {
        ...
        var enemyAction =
            enemy.CombatActions[
                Random.Shared.Next(enemy.CombatActions.Count)];

        combatManager.QueueCombatAction(playerAction, player);

        combatManager.QueueCombatAction(
            new CombatAction(
                enemyAction.Name,
                enemyAction.Power,
                enemy),
        ...
        if (result.Type == EventType.Loot)
        {
            ...
            Console.WriteLine(
                $"Victory! +{pts} score. Total: {player.Score}");

            gameManager.ApplyEffects(result, zoneManager);

            player.ClearEvent(gameEvent);
...
void Move()
{
    ...
    if (node?.Previous != null)
        Console.WriteLine($"  <- {node.Previous.Value.Name}");

    if (node?.Next != null)
        Console.WriteLine($"  -> {node.Next.Value.Name}");
