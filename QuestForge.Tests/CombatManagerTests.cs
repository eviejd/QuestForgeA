...
public class CombatManagerTests
{
    ...
    public void QueueAction_CanBeQueued()
    {
        ...
        Assert.True(cm.QueueCombatAction(action, player));
    }
    ...
    public void PlayRound_ReturnsLootWhenEnemyDies()
    {
        ...
        cm.QueueCombatAction(new CombatAction("Strike", 20, player), player);
        cm.QueueCombatAction(new CombatAction("Strike", 20, player), player);
    ...
    public void PlayRound_ReturnsGameOverWhenPlayerDies()
    {
        ...
        cm.QueueCombatAction(new CombatAction("Strike", 100, enemy), enemy);
    ...
    public void PlayRound_AccountsForDefence()
    {
        ...
        cm.QueueCombatAction(new CombatAction("Strike", 8, enemy), enemy);
        cm.QueueRoundOver(enemy);
    ...
    public void PlayRound_RoundOverBreaksLoop()
    {
        ...
        cm.QueueRoundOver(player);
        cm.QueueCombatAction(new CombatAction("Strike", 999, player), player);
    ...
    public void Flee_EndsCombat()
    {
        ...
        cm.QueueCombatAction(new CombatAction("Flee", 0, player), player);
        var result = cm.PlayCombatRound(player, enemy);
    ...
    [Fact]
    public void GetLog_ContainsEntriesAfterRound()
    {
        var cm = new CombatManager();
        var player = MakePlayer();
        var enemy = MakeEnemy();

        cm.QueueCombatAction(new CombatAction("Strike", 10, player), player);
        cm.QueueRoundOver(player);
        cm.PlayCombatRound(player, enemy);
